...
The legacy repair generator remains authoritative for its previously proven
patches. This file changes only the generated Playwright runtime boundary:

* build one derived Playwright image with Docker CLI and a `php` wrapper;
* pass that image identity into the frozen Platform validator container;
* mount the Docker socket and application container identity into each sample;
* replace per-sample Ubuntu PHP installation with a PHP 8.5 preflight.
"""
