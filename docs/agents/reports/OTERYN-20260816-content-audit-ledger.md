...
No new remediation issue/branch was scaffolded by this audit because the material implementation gaps found are already owned by #489, #301, #330, draft #338, #1114, or the owner-started #1115 programme, while production/runtime verification is outside this audit's authority.
...
| CA-006 | Player Companion Hunt Session Analyzer | `IMPLEMENTED`; authenticated owner-scoped CRUD, parser/validation and private no-store behavior; feature-test family present | `TERMINAL` | P2 | Current delivered tool count proven by routes/tests: 1 vertical slice (`session-analyzer`). User session record count is user/runtime data and not a completeness metric | current `main` | No content remediation required for this existing slice; retain regression coverage while adding independent tools. |
...
| CA-008 | Public/shared navigation | `IMPLEMENTED`: deterministic public navigation registry plus module fragments; Game Catalog/Wiki/Downloads/Events/Marketplace/Support have dedicated navigation fragments | `TERMINAL` | P2 | Static navigation presence verified; runtime-enabled conditional entries may depend on module configuration | current `main`; marketplace config for bazaar | Keep navigation coverage aligned with reachable route/config state. No separate content task needed. |
| CA-009 | Account, account security, registration/login/recovery/MFA | `IMPLEMENTED`: routes and focused feature-test families exist | `TERMINAL` | P2 | Static workflows are not content-count targets | current `main` | No #1115 content remediation. Preserve account/security test coverage and product ownership boundaries. |
...
| CA-011 | Announcements | Admin authoring module exists; landing/Today consumption is separate; standalone public route is not proven by the audited module file | `OWNED` | P2 | Database/runtime announcement count `UNKNOWN` | #1114 for Today consumption | Let #1114 own Today composition. Verify any intended standalone public announcement route separately before claiming it exists. |
| CA-012 | Events | Public list/detail + admin authoring/status routes exist; runtime event count unknown | `OWNED` | P2 | Public content count `UNKNOWN` | #1114 consumes Events in Today; existing Events module owns CRUD | Do not duplicate module work; verify live records in authorized runtime. |
...
| CA-015 | Homepage templates | Protected admin preview/activate/rollback routes exist; selected production template is runtime state | `TERMINAL` | P3 | Active production template `UNKNOWN` | PublicPortal/homepage template module; #1114 must avoid overlap | No #1115 content task. Runtime template choice belongs to authorized portal operations. |
...
- **Draft PR #1114** — open, unmerged and currently owns PublicPortal Today runtime paths; independent unless a content worker overlaps those paths.
...
Result: **zero new remediation scaffolds created**. Creating duplicates would violate live ownership.
...
1. Keep #1114 and #338 path ownership isolated.
...
6. Route any proposal to make CrystalServer-derived material a durable native authority through the architecture-review path instead of relabeling it as native truth.
