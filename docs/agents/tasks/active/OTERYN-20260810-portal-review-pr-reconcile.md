...
- [x] Exact-head self-review is complete and runtime/browser E2E is `NOT_APPLICABLE` for this documentation reconciliation.
- [ ] Repository-required exact-head CI passes, PR #964 merges and this correction task is archived with ownership released.
...
policy_version: 2
updated_at: 2026-08-10T19:16:00Z
head: 40307f72be8c21a3bcc7638cf9402cd3c13ca6a0
...
proven:
  ...
  - Current open PR refresh returns #961, #541 and #338 apart from this correction PR.
  ...
  - PR #964 changes exactly the dated report, delivery plan, archived original programme task and this active correction task.
derived:
...
unknown:
  - Exact final repository-required CI outcome for the final correction head until GitHub Actions completes.
conflicts:
  - The merged dated portal report and delivery plan said only #541 and #338 were open at the refresh; this correction resolves that conflict by recording #961 as already open.
first_failure:
...
validation:
  ...
  - command: PR #964 exact changed-path and full-diff self-review
    result: PASS
    evidence: GitHub changed-file and per-file patch inspection shows only the four declared documentation/governance paths, with the portal architecture conclusion unchanged and the inaccurate PR inventory corrected.
  ...
  - command: repository-required exact-head GitHub Actions
    result: NOT_RUN
    evidence: final checkpoint transition must complete exact-head workflows before merge.
blockers: []
next_action: Run and review exact-head PR #964 GitHub Actions on the final head, remediate any failure, then merge and archive this correction task.
```
