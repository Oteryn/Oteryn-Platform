...
- [ ] Accepted ADR 0031 records the native integration / legacy compatibility boundary and supersedes stale native protocol ownership decisions where necessary.
- [ ] A focused canonical `OTERYN_V2_INTEGRATION_ARCHITECTURE.md` records current-vs-target topology, ownership, persistence, admission, projections, analytics, compatibility, migration and risks.
- [ ] `ARCHITECTURE_AUTHORITY.md` routes this concern to the focused canonical document.
- [ ] ADR registry inventory is reconciled.
- [ ] The architecture review report preserves open-PR classifications and deferred P1/P2 questions without turning them into implementation authority.
...
updated_at: 2026-08-08T01:00:07+02:00
head: UNKNOWN
...
conflicts:
  - ADR 0010 and the Platform-native gameplay protocol contract currently assign native gameplay protocol selection/semantics more broadly to Platform/Gateway than the accepted target ownership model.
first_failure:
  marker: none
  evidence: none
rejected_hypotheses:
...
validation:
  ...
  - command: exact-head repository validation
    result: NOT_RUN
    evidence: coherent documentation package is still being created.
  - command: user/integration E2E
    result: NOT_APPLICABLE
    evidence: architecture/documentation-only task changes no executable journey.
blockers:
  - none
next_action: Create ADR 0031, the focused integration architecture and authority/report/registry reconciliation on this branch, then open the bounded PR.
```
