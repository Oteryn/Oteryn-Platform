...
proven:
  ...
  - no overlapping open PR owns Announcements paths
unknown:
  - first exact-head browser result for the composed Announcements matrix
conflicts: []
first_failure:
  marker: none
  evidence: initial package has not yet run
rejected_hypotheses:
...
blockers:
  - none
next_action: Open a draft PR and execute the exact-SHA Announcements workflow while the ledger remains planned.
```
