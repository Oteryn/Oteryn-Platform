...
A worker-session end, commit, PR creation, green CI, merge, E2E result, PR cleanup, task archive, or task handoff is a milestone, not by itself a reason to stop the owner invocation. No work continues after the final response; this instruction does not authorize hidden background execution.
