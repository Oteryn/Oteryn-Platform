...
| P1 | Production topology and OperationsObservability repository evidence | `OPEN` | `ARCHITECTURE_COORDINATOR` then `IMPLEMENTATION_OWNER` for bounded repository-safe slices | Medium | Issue #490 is the shared audit owner. Repository contracts/topology/observability/restore/rollback evidence may progress without claiming production state. |
...
| P2 | Client Distribution hardening | `OPEN` | `ARCHITECTURE_COORDINATOR` for trust/signature contract, then `IMPLEMENTATION_OWNER` | High after contract | Decide immutable/signed update manifest, stable/beta channels, minimum supported version, mandatory-update semantics and withdrawal/revocation before launcher launch claims. |
| P2 | Platform API | `ARCHITECTURE_READY` / `DECISION_REQUIRED` for first public surface | `ARCHITECTURE_COORDINATOR` | Medium after consumer decision | Issue #490 is the shared audit owner. Decide first concrete consumers, versioning, authentication, rate limits and compatibility; API remains an adapter over module application services. |
...
- Issue #490 is intentionally split by concern: Platform API, OperationsObservability/repository readiness and PublicEdge live proof have separate rows and may have different readiness/authority states.
