...
The approval file is removed in the evidence package so future `main` pushes cannot repeat this one-time cleanup. A fresh read-only inventory is required for any later cleanup batch.
