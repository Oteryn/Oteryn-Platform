...
CORE_DEPLOY="$SCRIPT_DIR/deploy.sh"
...
[[ -f "$CORE_DEPLOY" ]] || { echo "Canonical deployment script is missing: $CORE_DEPLOY" >&2; exit 1; }
...
# Presentation-only changes stay on deploy.sh's existing lighter platform-fast path.
...
# canonical in deploy.sh. Delegation occurs before any mutation here.
