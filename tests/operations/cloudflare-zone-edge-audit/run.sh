...
export CLOUDFLARE_API_TOKEN='cfat_mock-secret-value'
...
jq -e '
    ...
    and .certificates.login_hostname_covered == "false"
...
if grep -F 'cfat_mock-secret-value' "$TMP_DIR/audit.json" "$TMP_DIR/calls.log" || grep -F 'cfat_mock-secret-value' <<<"$output"; then
