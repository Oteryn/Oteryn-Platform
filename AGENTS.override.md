...
This root bootstrap is automatically loaded by Codex. It supplements and never weakens system, developer, owner, repository-allowlist, safety, production, credential, database, payment, authentication, deployment, merge, or cross-repository restrictions.
...
4. For any start, resume, continuation, autonomous-programme, or multi-task request, read `docs/agents/AUTONOMOUS_PROGRAM_CONTINUATION.md` before acting.
5. Inspect the authoritative active task checkpoint, live branch/head, related PRs, reviews, CI, ownership, dependencies, and current repository state. Do not reconstruct available state from chat history or ask the owner to repeat it.
6. If a required bootstrap document is missing or materially conflicts with live repository safety, stop and report the exact conflict instead of silently ignoring the document.
