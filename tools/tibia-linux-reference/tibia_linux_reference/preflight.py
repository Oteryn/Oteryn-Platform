...
REQUIRED_COMMANDS = ("git", "readelf", "sha256sum", "unshare")
...
def _storage_details(path: Path) -> dict[str, object]:
    ...
    if completed.returncode == 0:
        try:
            ...
            pass
    encrypted = fstype in {"ecryptfs", "encfs", "gocryptfs", "cryfs"} or source.startswith("/dev/mapper/")
...
def _network_isolator() -> dict[str, object]:
    ...
    if sudo:
        uid = os.getuid()
        gid = os.getgid()
        prefix = [sudo, "-n", "unshare", "-n", "--setuid", str(uid), "--setgid", str(gid), "--"]
