            ...
            <div class="mobile-nav-panel">
                <nav aria-label="{{ __('public.navigation.mobile') }}">
                    @foreach ($headerItems as $item)
                        @php($active = request()->routeIs(...explode('|', $item['active'])) || ($item['active'] === 'home' && request()->routeIs('localized.home')) || request()->routeIs(...array_map(static fn (string $pattern): string => 'legacy.'.$pattern, explode('|', $item['active']))))
