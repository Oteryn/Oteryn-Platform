...
- Organization semantic `WAITING_EXTERNAL` maps to Platform checkpoint `status: waiting`: persist the exact waiting event and one `next_action`, then end/release the active worker instead of polling. Organization semantic `STALLED` maps to fail-closed `status: blocked` when the unchanged retry budget is exhausted and no new hypothesis or evidence exists.
- For new or materially updated substantial Platform checkpoints that enter bounded validation/wait/retry handling, persist `candidate_frozen`, `candidate_head_sha`, `progress_fingerprint`, `failure_fingerprint`, `identical_cycle_count`, `retry_count`, `retry_limit`, `waiting_for` and `last_material_progress_at` as additive repository-specific evidence; untouched legacy checkpoint-version-1 records remain valid.
- Empty commits, semantic no-op edits, checkpoint-only churn and unrelated documentation mutations whose only purpose is to retrigger an external system are forbidden. Re-evaluate the same exact head when supported.
- Repeated unchanged failures use the repository anti-stall counters/budgets and stable material progress/failure identity; budget exhaustion never becomes a new candidate generation by itself.
- Waiting/stalled semantics never satisfy merge readiness. `platform-gate`, review hygiene and all other applicable exact-head fail-closed rules remain authoritative.
